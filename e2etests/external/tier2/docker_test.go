...
//
// Run with: go test -tags=docker ./e2etests/external/tier2/...
package tier2
...
// skipping the test if it is not configured.
func sandboxHostURL(t *testing.T) string {
	t.Helper()
	url := os.Getenv("SANDBOX_HOST_URL")
	if url == "" {
		t.Skip("SANDBOX_HOST_URL not set; skipping Docker-gated test")
	}
	return url
}
