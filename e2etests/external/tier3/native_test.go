...
//
// Run with: go test -tags=native ./e2etests/external/tier3/...
package tier3
...
import (
	...
	"h2-agent-runtime/e2etests/external/common"
)
...
// scenario against the real Anthropic API. The test is skipped if the
// ANTHROPIC_API_KEY environment variable is not set.
func TestNativeProviderIntegration_Anthropic(t *testing.T) {
	if os.Getenv("ANTHROPIC_API_KEY") == "" {
		t.Skip("ANTHROPIC_API_KEY not set; skipping Anthropic provider integration test")
	}
...
// scenario against the real OpenAI API. The test is skipped if the
// OPENAI_API_KEY environment variable is not set.
func TestNativeProviderIntegration_OpenAI(t *testing.T) {
	if os.Getenv("OPENAI_API_KEY") == "" {
		t.Skip("OPENAI_API_KEY not set; skipping OpenAI provider integration test")
	}
