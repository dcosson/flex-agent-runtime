...
.PHONY: help build build-llm-demo build-embedding-demo build-stubserver build-sandbox-host build-flexagent fmt fmt-check vet deps deps-staticcheck check test test-race test-integration test-ai-clients-real-apis test-e2e-mock test-e2e-real-apis \
...
help:
	...
	@echo "  test-ai-clients-real-apis        Go live API tests for ai/ library (real providers, requires API keys)"
	@echo "  test-e2e-mock                    Python E2E mock tier tests (stubserver, no API keys needed)"
	@echo "  test-e2e-real-apis               Python E2E real tier tests (real providers, requires API keys + infra)"
	@echo ""
	@echo "=== Harness Tests ==="
...
test-ai-clients-real-apis:
	$(GO) test -tags=liveapi -v -count=1 -timeout 300s ./tests/liveapi/...

test-e2e-mock:
	cd tests/e2e && pip install -q -r requirements.txt && pytest -m "not real" -v --timeout 120

test-e2e-real-apis:
	cd tests/e2e && pip install -q -r requirements.txt && pytest test_provider_api.py test_deployment_modes.py test_sandbox_config.py test_zfs_durability.py test_sandbox_identity.py test_multi_runtime.py -v --timeout 300
