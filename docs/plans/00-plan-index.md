# Pi Agent Go — Plan Index

## Overview

This plan index organizes the V1 implementation of pi-agent-go into sub-plans grouped by dependency order. The project has two packages (`piai` and `piagent`) with clear layering — `piai` is the foundation, `piagent` builds on top.
...
|------|----------|---------|
| **G1: Core types compile** | `piai` types, event stream, and registry compile. Model catalog loads. No providers yet. | Batch 1 → Batch 2 |
...
| **G4: Agent loop works** | `piagent.Agent` can run a multi-turn conversation with tools using any V1 provider. Steering and follow-up messages work. | Batch 3 → Done |
...
| Doc | Component | Description | Depends On | Status |
|-----|-----------|-------------|------------|--------|
| [02-provider-anthropic](./02-provider-anthropic.md) | Anthropic provider | Anthropic Messages API: message conversion, SSE stream parsing, thinking (adaptive + budget), tool calling, cache control, cost tracking. First provider — validates the core architecture. | 01-piai-core | Not started |
| [03-provider-openai](./03-provider-openai.md) | OpenAI provider | OpenAI Completions API: message conversion, SSE parsing, reasoning effort mapping, tool calling, compatibility settings for OpenAI-compatible endpoints (Groq, Mistral, etc.) | 01-piai-core, 02-provider-anthropic | Not started |
| [04-provider-google](./04-provider-google.md) | Google provider | Google Generative AI (Gemini): message conversion, SSE parsing, thinking, tool calling, thought signatures. | 01-piai-core, 02-provider-anthropic | Not started |
...
| Doc | Component | Description | Depends On | Status |
|-----|-----------|-------------|------------|--------|
| [05-piagent](./05-piagent.md) | `piagent` package | Agent struct, agent loop (tool execution, steering, follow-up), AgentMessage/AgentTool/AgentEvent types, subscription model, state management. Full unit tests with mock provider, integration test with real provider. | 01-piai-core, 02-provider-anthropic | Not started |

## Dependency Graph

```mermaid
graph TD
    A[01-piai-core<br/>Core types, EventStream,<br/>registries, models, validation]
    ...
    E[05-piagent<br/>Agent loop, tools,<br/>steering, follow-up]
...
2. **Model catalog maintenance**: How to keep the Go model catalog in sync with the TS `models.generated.ts`? Options: (a) write a Go code generator that reads the same sources, (b) periodically copy and convert, (c) embed the TS-generated JSON. Affects 01-piai-core.
