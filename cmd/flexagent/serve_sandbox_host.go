...
func parseSandboxHostConfig(args []string) sandboxHostConfig {
	var cfg sandboxHostConfig
	fs := flag.NewFlagSet("flexagent serve sandbox-host", flag.ExitOnError)

	fs.StringVar(&cfg.ListenAddr, "listen", envOrDefault("SANDBOX_HOST_LISTEN", ":8080"), "listen address")
	...
	fs.IntVar(&cfg.MaxSessions, "max-sessions", envIntOrDefault("SANDBOX_MAX_SESSIONS", 0), "maximum live sessions (0 = unlimited)")
	...
	fs.IntVar(&cfg.RPCMaxMessageBytes, "rpc-max-message-bytes", envIntOrDefault("SANDBOX_RPC_MAX_MESSAGE_BYTES", 16<<20), "max rpc message size in bytes")
	fs.StringVar(&cfg.APIVersion, "api-version", envOrDefault("SANDBOX_API_VERSION", "v1"), "advertised api version")
	fs.StringVar(&cfg.MinAPIVersion, "min-api-version", envOrDefault("SANDBOX_MIN_API_VERSION", "v1"), "minimum supported api version")
	fs.StringVar(&cfg.AuthToken, "auth-token", envOrDefault("SANDBOX_AUTH_TOKEN", ""), "bearer auth token; empty disables auth")
...
func (cfg sandboxHostConfig) validate() error {
	var errs []error
...
func runServeSandboxHost(args []string) {
	...
	logger.Info("flexagent serve sandbox-host starting", "listen", cfg.ListenAddr, "api_version", cfg.APIVersion, "terminal_enabled", cfg.EnableTerminal)
	go func() {
	...
	logger.Info("flexagent serve sandbox-host shutdown complete")
}
